...
crvs := eval Read("15-11.m");

k := 1;

//for crv in crvs[1..#crvs] do
for i in [4..#crvs] do
  crv := crvs[i];
  m := crv[1]; m;
  C := Curve(A2, crv[k+1]);
  ...
  P<[x]> := FunctionField(Rationals(), Dimension(AmbientSpace(X)) + 1);
  
  m1 := [P | LiftPolyViaLLL(f) : f in m1]; m1;
  m2 := [P | LiftPolyViaLLL(f) : f in m2]; m2;

  /* m := [m1[1]/m1[2], m2[1]/m2[2]]; m; */
